...
if __name__ == "__main__":
    test_rooms = [
        {"name": "Living Room", "width": 6, "length": 12},   # load-bearing, medium confidence
        {"name": "Kitchen", "width": 3, "length": 4},         # fire-sensitive, high confidence
        {"name": "Hall", "width": 8, "length": 18},           # load-bearing, low confidence
        {"name": "Store Room", "width": 2, "length": 3},      # partition, high confidence
        {"name": "Bedroom 1", "width": 4, "length": 5},       # load-bearing, high confidence
        {"name": "Office", "width": 5, "length": 11, "load_bearing": False},  # explicit override
    ]

    for room in test_rooms:
        analysis = analyze_wall(room)
        lb = "⚙️ load-bearing" if analysis["load_bearing"] else "🧱 partition"
